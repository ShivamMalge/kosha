...
| F4.2 | Token cost of a correct non-fire is at most **300 tokens** above the skill-off baseline. Measured cost is ~260 (gate 246 + verdict line ~14), so the bound carries **~15% headroom** — enough for wording drift, not enough to absorb a regression. A bound of 400 was rejected: it would have swallowed a 50% growth in gate size without firing, which looks measured and is not. | Fresh, cache-read and output token deltas, `benchmark.md` §7.4 |
